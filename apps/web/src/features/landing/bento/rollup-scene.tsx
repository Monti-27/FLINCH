...
export function RollupScene({ phase, transition }: SceneMotion) {
  ...
  return <div className={styles.scene} data-rollup-scene="" data-rollup-step={phase} data-bento-contained="">
    ...
    <div className={styles.bridge}>
      <span className={styles.rail} /><span className={styles.rail} />
      ...
      <motion.div className={styles.control} data-rollup-packet="" data-bento-contained="" initial={false}
        ...
        <div className={styles.steps}>
          {["Queue", "Batch", "Return"].map((label, index) => <span key={label}>
            <motion.i initial={false} animate={{ opacity: index === Math.max(0, phase - 1) ? 1 : 0 }} transition={transition()} />
            <span>{label}</span>
          </span>)}
        </div>
      </motion.div>
