...
type Props = { room?: BaseRoom; control?: Control; seat?: number; remaining?: bigint; remainingMs?: bigint; stale?: boolean };
...
export function Standoff({ room, control, seat = -1, remaining = 90n, remainingMs = remaining * 1000n, stale = false }: Props) {
  ...
  return <section className={`standoff ${styles.bar}`} data-room={!!room} aria-label="Round overview">
    ...
    <div className={styles.stats}>
      ...
      <RoundTimer key={room?.ledger.address.toBase58() ?? "lobby"} milliseconds={remainingMs} funding={funding} ended={ended} stale={stale} />
    </div>
  </section>;
}
